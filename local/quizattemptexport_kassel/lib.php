...
function local_quizattemptexport_kassel_extend_settings_navigation(\settings_navigation $settingsnav, $context) {
    ...
    if (!has_any_capability(array('local/quizattemptexport_kassel:viewpdf', 'local/quizattemptexport_kassel:generatepdf'), $context)) {
        return;
    }
 ...
 */
function local_quizattemptexport_kassel_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options=array()) {
    ...
    require_login();
    require_capability('local/quizattemptexport_kassel:viewpdf', $context);
    ...
    $fs = get_file_storage();
    $file = $fs->get_file($context->id, 'local_quizattemptexport_kassel', $filearea, $itemid, $filepath, $filename);
    if (!$file) {
